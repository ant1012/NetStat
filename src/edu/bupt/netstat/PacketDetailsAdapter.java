...
public class PacketDetailsAdapter extends ArrayAdapter<JPacket> {
	 ...
	 * 
	 */
	public PacketDetailsAdapter(Context context, int resourceId,
			List<JPacket> objects, PacketReader reader) {
	...
	private void setValue(ViewHolder vh, int position) {
		JPacket p = getItem(position);
		if (p != null) {
			vh.packet.setText(Integer.toString(position + 1));
			vh.len.setText(Integer.toString(p.getPacketWirelen()));
			if (reader.rttMap.containsKey(position)) {
				vh.delay.setText(reader.rttMap.get(position).toString());
			} else {
				vh.delay.setText("---");
			}
			if (reader.retTable[position]) {
